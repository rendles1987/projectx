...
class RawCsvInfo:
    ...
    def _update_csv_info(self):
        ...
        for csv_full_filepath in self._get_all_csvs_from_dir_only_path(self.cup_dir):
            self._csv_info.append(("cup", csv_full_filepath))
        for csv_full_filepath in self._get_all_csvs_from_dir_only_path(self.league_dir):
            self._csv_info.append(("league", csv_full_filepath))
        for csv_full_filepath in self._get_all_csvs_from_dir_only_path(self.player_dir):
    ...
    def _get_not_checked_csvs_from_dir(folder_dir):
        ...
        csv_paths = [
            ...
            and not file.endswith("_invalid.csv")
            and not file.endswith("_valid.csv")
        ]
        return csv_paths

    @staticmethod
    def _get_all_csvs_from_dir_only_path(folder_dir):
        """ get all .csvs - that have no 'valid' or 'invalid' in filename -
        from a directory
        :param folder_dir: string of dir's full path
        :return: csv_paths (list with strings)"""
